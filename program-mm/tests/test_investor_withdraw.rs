...
use mm::{
    ...
    instruction:: {init_mm_fund, investor_deposit, process_deposit, investor_request_withdraw, process_withdraw, investor_withdraw},
};
...
async fn test_mm_fund() {
    ...
    //todo
    // let vault_account = await createAssociatedTokenAccountIfNotExist(walletProvider, new PublicKey('8FRFC6MoGGkMFQwngccyu69VnYbzykGeez7ignHVAFSN'), mango_group_ai, transaction);
    ...
    let signer_account = Pubkey::from_str("9BVcYqEQxyccuwznvxXqDkSJFavvTyheiTYk231T1A8S").unwrap();
    ...
    // investor withdraw request
    {
        let mut transaction = Transaction::new_with_payer(
            &[
                investor_request_withdraw(
                    &program_id,
                    &fund_pda,
                    // todo: investor state account
                    &investor.pubkey(),
    ...
    // process withdraw
    {
        ...
        // Test expected amount is added to fund vault
        let final_fund_balance = get_token_balance(&mut banks_client, fund_usdc_vault.pubkey).await;
        assert_eq!(final_fund_balance, deposit_amount);

        let mango_account_balance = get_token_balance(&mut banks_client, mango_account_ai).await;
        assert_eq!(mango_account_balance, 0);
    }
    ...
    // investor withdraw
    {
        ...
        // Test expected amount is added to investor wallet
        let final_investor_balance = get_token_balance(&mut banks_client, investor_usdc_account.pubkey).await;
        assert_eq!(final_investor_balance, initial_amount);

        let fund_vault_balance = get_token_balance(&mut banks_client, fund_usdc_vault.pubkey).await;
        assert_eq!(fund_vault_balance, 0);
