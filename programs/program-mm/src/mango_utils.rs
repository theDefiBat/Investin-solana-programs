...
use mango::instruction::{
    cancel_perp_order_by_client_id, deposit, place_perp_order, withdraw, MangoInstruction,
};
...
pub fn mango_deposit(
    ...
    check!(
        manager_acc.is_signer,
        ProgramError::MissingRequiredSignature
    );

    // check_eq!(fund_data.manager_account, *manager_acc.key);
    check!(
        (fund_data.manager_account == *manager_acc.key) || (fund_data.delegate == *manager_acc.key),
        FundError::ManagerMismatch
    );
...
pub fn mango_withdraw(
    ...
    check!(
        (fund_data.manager_account == *manager_ai.key) || (fund_data.delegate == *manager_ai.key),
        FundError::ManagerMismatch
    );
...
pub fn get_mngo_accrued(
    ...
    let mngo_deposits =
        mango_account.get_native_deposit(&mango_cache.root_bank_cache[mngo_index], mngo_index)?;
